<?php $this->titre = 'Modifier ' . $langageDeProgrammation['nom']; ?>
...
<form action="index.php?action=mettre_a_jour&id=" method="post">
    ...
    <p>
        <label for="nom">Langage de programmation</label> : <input type="text" name="nom" id="nom" value="<?= $langageDeProgrammation['nom'] ?>" /><br />
        <label for="description">Description</label> : <textarea name="description" id="description"><?= $langageDeProgrammation['description'] ?></textarea><br />
...
<!--        --><?//= ($erreur == 'url') ? '<span style="color : red;"> Entrez un URL valide</span>' : '' ?><!-- </br>-->
        ...
        <input type="hidden" name="cours_id" value="<?= $langageDeProgrammation['cours_id'] ?>" /><br />
